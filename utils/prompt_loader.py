...
def load_system_prompts():
    try:
        ...
        logger.error("[load_system_prompts]yaml配置项中没有main_prompt_path配置")
        raise e

    try:
        return open(system_prompt_path,"r",encoding="utf-8").read()
    except Exception as e:
        logger.error(f"[load_system_prompts]解析报告提示词出错，{str(e)}")
        raise e
...
def load_rag_prompts():
    try:
        ...
        logger.error("[load_rag_prompts]yaml配置项中没有rag_summarize_prompt_path配置")
        raise e

    try:
        return open(rag_prompt_path,"r",encoding="utf-8").read()
    except Exception as e:
        logger.error(f"[load_rag_prompts]解析报告提示词出错，{str(e)}")
        raise e
...
def load_report_prompts():
    try:
        ...
        logger.error("[load_report_prompts]yaml配置项中没有report_prompt_path配置")
        raise e
